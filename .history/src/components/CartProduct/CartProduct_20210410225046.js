...
export default function CartProduct() {
  ...
  // dispatch
  const deleteToCart = (cart) => {
    //   dispatch(actionTypes.deleteToCart(cart));
  };
  const updateQuantity = (index, quantity) => {
    //   dispatch(actionTypes.updateQuantity(index, quantity));
  };
  const addToCartAPI = (data) => {
    // dispatch(actionTypes.addToCartRequestAPI(data));
  };
              ...
              {/* {data.map((card, index) => (
                <div className="card-iteml" key={index}>
                  ...
                  <div className="card-name">
                    <Link
                      title="Xem chi tiết"
                      to={`/${card.product.key}/${card.product.NSX.replace(
                        / /g,
                        "-"
                      )}/${card.product.name.replace(/ /g, "-")}/${
                        card.product._id
                      }`}
                      onClick={() => {
            ...
            {/* <div className="card-total-money">
              <div className="totle-money">
                ...
                <div className="group-totle-money">
                  <p>
                    {formatter.format(showTotalAmount(data))} <u>đ</u>
                  </p>
    ...
    <>
      <div className="container-card">
        <div className="group-card">
          <h3>
            {/* GIỎ HÀNG <span>({dataCart.length} sản phẩm)</span> */} giỏ hàng
          </h3>
